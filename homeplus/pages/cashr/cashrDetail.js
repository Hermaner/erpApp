		...
		mui("#dismore").on('tap', "li", function() {
			...
			if(pid == 'cp') {
				E.prompt('原订单金额：' + self.vue.totalPrice, '请输入修改金额', function(c) {
					...
					self.vue.entryCoupon = parseFloat(self.vue.totalPrice) - parseFloat(c)
				})
			} else if(pid == 'vp') {
				self.vue.goVipinfo()
			}
	...
	vueObj: {
		...
		computed: {
			...
			totalPrice: function() {
				...
				var dprice = (this.discount * 100 * tprice) / 1000 //优惠折扣后的订单总价
				...
				var coupons = parseFloat(dprice3) + parseFloat(this.denomination) + parseFloat(dprice2) + parseFloat(this.entryCoupon)
