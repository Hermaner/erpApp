var chartPage = {
	...
	plusEvent: function() {
		...
		mui.plusReady(function() {
			...
			E.showLoading()
				//			window.addEventListener('chartShow', function(event) {
				//				var chartId = parseInt(event.detail.chartId);
				//				self.vue.chartId = chartId;
				//				self.vue.title = self.textAr[chartId][0];
				//				self.vue.name = self.textAr[chartId][1];
				//				self.vue.type = self.textAr[chartId][2];
				//				self.vue.loadData(self.vue.type, "60day")
				//				self.vue.loadData(self.vue.type, "yesterday");
				//				E.showLoading()
				//			})
	...
	clearData: function() {
		...
		mui("#main")[0].innerHTML = "";
		mui("button.mui-active")[0].classList.remove("mui-active")
		mui("button")[0].classList.add("mui-active")
		this.vue.items = [
				[],
				[],
				[],
			...
			this.vue.totalCount = "",
			this.vue.showpie = false,
			this.vue.famount = null,
			this.vue.tamount = null,
			chartData.xdata = [
				[],
				[],
				[],
			...
			chartData.allYdata = [],
			chartData.yeardata = [],
			chartData.yearCollection = [
				[]
			],
			chartData.xCollection = [
				[]
			];
	},
	vueObj: {
